...
namespace DMO.Models
{
    public abstract class MediaData : BaseModel
    {
        ...
        public async void TryRenameFileAsync(string newName)
        {
            ...
            if (await Task.Factory.StartNew(() => File.Exists(newPath)))
                return;
        ...
        public async Task EvaluateOnlineAsync(string firebaseToken)
        {
            ...
            using (new DisposableLogger(DatabaseLog.DeserializationBegin, (sw) => DatabaseLog.DeserializationEnd(sw, 1)))
            {
                ...
                imageReponse = await Task.Run(() => JsonConvert.DeserializeObject<AnnotateImageReponse>(result.Trim(trimChars)));
            }
